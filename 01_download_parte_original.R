...
get_foros <- function(inst) {
  id <- dplyr::case_when(
    ...
    inst == 2L ~ "id_Seção")
  u <- dplyr::case_when(
    inst == 1L ~ "https://esaj.tjsp.jus.br/cpopg/open.do",
...
#'
#' @export
download_cpopg_parte <- function(parte, path = ".", foro = "-1", nome_completo = FALSE) {

  # Download one lawsuit
  download_cpopg_parte_ <- function(page, url_encoded, path, foro, nome_completo) {
    ...
    if (length(links) != 0) {
      ...
      purrr::map2(links, f_lwst, ~httr::GET(.x, httr::config(ssl_verifypeer = FALSE), httr::write_disk(.y, TRUE)))
      ...
      httr::GET("https://esaj.tjsp.jus.br/cpopg/search.do", query = query, httr::config(ssl_verifypeer = FALSE), httr::write_disk(f_lwst, TRUE))
  ...
  if (stringr::str_detect(as.character(f_search), "muitos processos")) {
    stop("Too many lawsuits found! Please narrow your search with 'foro'.")
  }
  ...
  if (is.na(pages)) {
    results <- download_cpopg_parte_(1, url_encoded, path, foro, nome_completo)
  }
  else {
    results <- abjutils::pvec(1:pages, download_cpopg_parte_, url_encoded, path, foro, nome_completo)
...
#'
#' @export
download_cposg_parte <- function(parte, path = ".", foro = "-1", nome_completo = FALSE) {

  # Download one lawsuit
  download_cposg_parte_ <- function(page, url_encoded, path, foro, nome_completo) {
    ...
    if (length(links) != 0) {
      ...
      httr::GET("https://esaj.tjsp.jus.br/cposg/search.do", query = query, httr::config(ssl_verifypeer = FALSE), httr::write_disk(f_lwst, TRUE))
  ...
  # If theres only one page, just download, otherwise loop
  if (is.na(pages)) {
    results <- download_cposg_parte_(1, url_encoded, path, foro, nome_completo)
  }
  else {
    results <- abjutils::pvec(1:pages, download_cposg_parte_, url_encoded, path, foro, nome_completo)
    results <- purrr::flatten_chr(results$output)
  }
